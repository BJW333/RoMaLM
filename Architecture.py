...
class SelfAttentionWithRoPE(nn.Module):
    ...
    def forward(self, x, key_padding_mask=None):
        ...
        qkv = qkv.view(B, S, self.n_heads, 3, self.d_head)  # (B, S, n_heads, 3, d_head)
        qkv = qkv.permute(0, 2, 1, 3, 4)  # (B, n_heads, S, 3, d_head)
        q, k, v = qkv.unbind(3)  # Now (B, n_heads, S, d_head) each
        # apply RoPE to queries & keys
        q = self.rotary.rotate_queries_or_keys(q)
        k = self.rotary.rotate_queries_or_keys(k)
        # output: (B, heads, S, d_head)
        # Build causal mask: shape (S, S), True=keep, False=mask
        causal_mask = torch.tril(torch.ones(S, S, dtype=torch.bool, device=x.device))
        if key_padding_mask is not None:
            pad_mask = key_padding_mask[:, None, None, :].expand(-1, self.n_heads, S, -1)
            causal_mask = causal_mask.unsqueeze(0).unsqueeze(0)  # (1,1,S,S)
            attn_mask = ~causal_mask | pad_mask  # True = masked
        else:
            attn_mask = ~causal_mask  # True = masked
        # scaled dot-prod attention
        attn_out = F.scaled_dot_product_attention(
            q, k, v,
            attn_mask=attn_mask,
            dropout_p=self.dropout_p,
            is_causal=False  # We do our own causal masking
        )
...
class ModernTransformerLM(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        ...
        if labels is not None:
            # Set pad tokens in labels to -100 for ignore_index
            labels = labels.clone()  # Avoid modifying input in-place
            ...
            loss = F.cross_entropy(
                ...
                labels.view(-1),
                ignore_index=-100 #set to -100 for padding tokens
            )
            return logits, loss
        return logits
